...
Here are the options currently supported:
  ...
  Usage: tack [options] [file]
      ...
      -n, --name PATTERN               Run only tests that match pattern.
      ...
      -s, --shuffle [RUNS]             Run tests in randomized order RUNS times (defaults to 1).
...
* Per-project configuration file (.tack) for configuring middleware (like a rackup file)
* Rake tasks
